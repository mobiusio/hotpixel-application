...
#include "ui.h"													/* Defines hooks into vcom structures */
#include <stdio.h>
...
/* Receive Buffer */ // no longer static to give ui.com access
uint_8 g_curr_recv_buf[DATA_BUFF_SIZE];
/* Send Buffer */
uint_8 g_curr_send_buf[DATA_BUFF_SIZE];
/* Receive Data Size */
static uint_32 g_recv_size;
/* Send Data Size */
static uint_32 g_send_size;
/*****************************************************************************
 * Local Functions
 *****************************************************************************/
 /******************************************************************************
...
static void Virtual_Com_App(void)
{
                ...
                if (g_recv_size) {
                                g_recv_size = 0;
                                ui_handle_input(g_curr_recv_buf, &ptr, &g_send_size);
                }
...
)
{
                                                ...
                                                (start_transactions == TRUE))
                {
                                ...
                                uint_8 index;
                                BytesToBeCopied = (USB_PACKET_SIZE)((dp_rcv->data_size > DATA_BUFF_SIZE) ? DATA_BUFF_SIZE:dp_rcv->data_size);
                                ...
                                //
                                // call vcom_main_task(); here so we don't wait for main loop,
																// need to treat vcom_main_task() like an interrupt service routine
																//
                                
																if (index) {
                                        vcom_main_task(); // also includes background CDC tasks
                                }
                }
