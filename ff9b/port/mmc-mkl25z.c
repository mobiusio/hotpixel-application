...
void xmit_spi (BYTE dat)
{
    ...
    //SSIDataGet(SDC_SSI_BASE, &rcvdat); /* flush data read during the write */
		...
		//spi1_blocking_xmit(&spi_buf, &dat, 1);
	  
		spi1_byte_xmit(dat, (uint_8 *) &rcvdat);
}
...
BYTE rcvr_spi (void)
{
    ...
    //SSIDataGet(SDC_SSI_BASE, &rcvdat); /* read data frm rx fifo */
		...
		//spi1_blocking_xmit(&spi_buf, &dummy_byte, 1);
		//rcvdat = spi_buf.rx_buf[0];
	
		spi1_byte_xmit(0xFF, (uint_8 *) &rcvdat);
...
void send_initial_clock_train (void)
{
    ...
    for(i = 0 ; i < 10 ; i++) {
        ...
        //SSIDataGet(SDC_SSI_BASE, &dat);
				//spi1_blocking_xmit(&spi_buf, &dummy_byte, 1);
				spi1_byte_xmit(0xFF, (uint_8 *) &dat);
    }
...
void set_max_speed(void)
{
    ...
    //SSIEnable(SDC_SSI_BASE);
		...
		//
		// TODO: ADD FUNCTION HERE, RUNNING AT 1MHZ NOW
		//
}
...
DWORD get_fattime (void)
{
	/*
		unsigned long mo = 0;
    unsigned long ut = utime + 36*SECONDS_IN_HOUR;
    unsigned long ye = ut/SECONDS_IN_YEAR;
    unsigned long da = (ut - ye*SECONDS_IN_YEAR)/SECONDS_IN_DAY;
    unsigned long ho = (ut - ye*SECONDS_IN_YEAR 
                          - da*SECONDS_IN_DAY)/SECONDS_IN_HOUR;
    unsigned long mi = (ut - ye*SECONDS_IN_YEAR
                            - da*SECONDS_IN_DAY
                              - ho*SECONDS_IN_HOUR)/SECONDS_IN_MINUTE;
    unsigned long se = (ut - ye*SECONDS_IN_YEAR
                            - da*SECONDS_IN_DAY
                              - ho*SECONDS_IN_HOUR
                                - mi*SECONDS_IN_MINUTE);
    
    da++; // to make sure we start with 1
    
    
    
    // calculate month based on day in year
    ye += 1970;
    if ((ye-2000)%4) { // remainder, so not a leap year
      if (da <= 31) {
        mo = 1;
      }
      else if (da <= 59) {
    ...
    // debugger only, will crash nomral operation
    // printf("%d-%d-%d %d:%d:%d\r\n", ye, mo, da, ho, mi, se);
